...
__global__ void SigmoidWeightedCrossEntropyLossForwardGPU(const int nthreads,
...
template <typename Dtype>
__global__ void SigmoidWeightedCrossEntropyLossBackwardGPU(const int nthreads,
...
template <typename Dtype>
void SigmoidWeightedCrossEntropyLossLayer<Dtype>::Forward_gpu(
  ...
  SigmoidWeightedCrossEntropyLossForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(count, input_data, target, loss_data);
...
void SigmoidWeightedCrossEntropyLossLayer<Dtype>::Backward_gpu(
  ...
  if (propagate_down[0]) {
    ...
    SigmoidWeightedCrossEntropyLossBackwardGPU<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(count, bottom_diff, target, (double)weight_pos_, (double)weight_neg_);
...
INSTANTIATE_LAYER_GPU_FUNCS(SigmoidWeightedCrossEntropyLossLayer);
