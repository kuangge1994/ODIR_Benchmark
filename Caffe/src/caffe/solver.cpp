  ...
  for (int i = 0; i < test_score.size(); ++i) {
    ...
    //      Dtype prob = test_score[i]->cpu_data()[j*num_columns + k] /
     //                     param_.test_iter(test_net_id);
	  ...
	  Dtype TN = test_score[i]->cpu_data()[0];
	  Dtype FP = test_score[i]->cpu_data()[1];
	  Dtype FN = test_score[i]->cpu_data()[2];
	  Dtype TP = test_score[i]->cpu_data()[3];
	  Dtype RE = TP / (TP+FN);
	  Dtype PR = TP / (TP+FP);
	  Dtype F1 = 2*RE*PR/(RE+PR);
      LOG(INFO) << "    Test net output #"<< i << ": F1-score " << F1; 
	  Dtype N = TP + FP + FN + TN;
	  Dtype p0 = (TP + TN) / N;
 	  Dtype pe = ((TN+FP)*(TN+FN) + (TP+FN)*(TP+FP)) / (N*N);
	  Dtype kappa = (p0 - pe) / (1 - pe);
      LOG(INFO) << "    Test net output #"<< i << ": kappa " << kappa; 
    } else {
      LOG(INFO) << "    Test net output #" << i << ": " << output_name
